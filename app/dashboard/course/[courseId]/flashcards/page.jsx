...
function Flashcards() {
  ...
  const GetFlashCards = async () => {
    try {
      ...
      console.log("📦 API flashcard data:", result.data);

      // If result.data is an array, extract the first item’s content
          ...
          <Carousel setApi={setApi} className="max-w-3xl mx-auto">
            <CarouselContent>
              {flashCards.content?.map((flashcard, index) => {
                ...
                const flashcardKey = `flashcard-${index}-${front?.substring(0, 10) || "f"}-${back?.substring(0, 10) || "b"}`;
                return (
                  <CarouselItem
                    key={flashcardKey}
